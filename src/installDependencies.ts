...
export const installDependencies = async (pmArg: PackageManager | undefined, settings: Settings) => {
	...
	if (settings.linter !== 'none') {
		...
		const linterCmd = settings.linter === 'biome' ? installBiomeCmd : installEslintCmd
